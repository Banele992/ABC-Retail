...
namespace ABCRetail.Controllers
{
    ...
    {
        private readonly CustomerService _tableStorageService;
        private readonly BlobStorageService _blobStorageService;
        private readonly QueueStorageService _queueStorageService;
        private readonly FileShareStorageService _fileShareStorageService;

        public CustomerController(CustomerService tableStorageService, BlobStorageService blobStorageService, QueueStorageService queueStorageService, FileShareStorageService fileShareStorageService)
        {
        ...
        public async Task<IActionResult> Create(Customer customer, IFormFile photo)
        {
            if (ModelState.IsValid)
            {
                ...
                if (photo != null)
                {
                    ...
                    customer.CustPhoto = await _blobStorageService.UploadPhotoAsync(Guid.NewGuid().ToString(), stream);
                }
        ...
        public async Task<IActionResult> Edit(Customer customer, IFormFile photo)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    // Retrieve the existing customer from Table Storage
                    ...
                    if (photo != null && photo.Length > 0)
                    {
                        ...
                        if (!string.IsNullOrEmpty(existingCustomer.CustPhoto))
                        {
                            var blobName = Path.GetFileName(new Uri(existingCustomer.CustPhoto).AbsolutePath);
                    ...
                    else
                    {
                        ...
                        customer.CustPhoto = existingCustomer.CustPhoto;
                    }

                    // Update the customer in Table Storage
        ...
        public async Task<IActionResult> DeleteConfirmed(string partitionKey, string rowKey)
        {
            ...
            if (customer != null && !string.IsNullOrEmpty(customer.CustPhoto))
            {
                var blobName = Path.GetFileName(new Uri(customer.CustPhoto).AbsolutePath);
                await _blobStorageService.DeletePhotoAsync(blobName);
            }
...
//Reference List:
//Microsoft.[n.d.]. Introduction to Azure storage, [n.d.]. [Online]. Available at: https://learn.microsoft.com/en-us/azure/storage/common/storage-introduction [Accessed 20 August 2025].

//Microsoft.[n.d]. Sopping Cart [n.d]. [Online]. Available at: Shopping Cart | Microsoft Learn
// [Accessed 20 May 2025].

//ASP.NET Core MVC Tutorial – Build a Website Shopping cart. 2023. YouTube video added by Evan Gudmestad. [Online]. Available at: https://youtu.be/PwQyRQuEor0?si=3et8vPiidJnFwz9-  [Accessed 23 May 2025].

//W3Schhols. [n.d.]. HTML, [n.d.]. [Online]. Available at: HTML Tutorial (w3schools.com) [Accessed 243May 2024].

//Shopping Cart project in .net core mvc (with authentication). 2023. YouTube video added by Ravindra Devrani. [Online]. Available at: https://youtu.be/JPFlSXejgKc?si=VXSsHSydovhZY3TA  [Accessed 22 May 2025].
...
//W3Schhols. [n.d.]. HTML, [n.d.]. [Online]. Available at: HTML Tutorial (w3schools.com) [Accessed 24 May 2024].
